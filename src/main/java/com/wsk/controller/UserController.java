...
import com.wsk.tool.VerificationCodeUtil;
...
public class UserController {
    ...
    public Map<String, Object> checkCode(HttpServletRequest request,
        ...
        // 验证码为空或不匹配
        if (StringUtils.getInstance().isNullOrEmpty(sessionCode) || !sessionCode.equalsIgnoreCase(code)) {
            map.put("result", 0);
            return map;
        }
        ...
        // 如果是注册操作，需要检查手机号是否已被注册
        if (!StringUtils.getInstance().isNullOrEmpty(name)) {
            if (userInformationService.selectIdByPhone(phone) != 0) {
                map.put("result", -1); // 手机号已被注册
                return map;
            }
            // 将用户名存入session
            ...
            // 如果是忘记密码操作，需要检查手机号是否存在
            if (userInformationService.selectIdByPhone(phone) == 0) {
                map.put("result", -1); // 手机号不存在
                return map;
            }
        }
    ...
    @ResponseBody
    public Map<String, String> getVerificationCode(HttpServletRequest request) {
        VerificationCodeUtil.VerificationCode verificationCode = VerificationCodeUtil.generateVerificationCode();
        // 将验证码存入session
        request.getSession().setAttribute("verificationCode", verificationCode.getCode());
        // 在控制台输出验证码，方便测试
        log.info("生成的验证码为: " + verificationCode.getCode());

        Map<String, String> result = new HashMap<>();
        result.put("image", verificationCode.getImageBase64());
        return result;
    }
    ...
    public String registered(Model model,
        ...
        if (StringUtils.getInstance().isNullOrEmpty(sessionVerificationCode) ||
                !sessionVerificationCode.equalsIgnoreCase(verificationCode)) {
            model.addAttribute("result", "verification_code_error");
            return "success";
        }
        // 清除session中的验证码
        request.getSession().removeAttribute("verificationCode");
        ...
        // 验证手机号是否已注册
        if (userInformationService.selectIdByPhone(phone) != 0) {
            model.addAttribute("result", "phone_exists");
            return "success";
        }
